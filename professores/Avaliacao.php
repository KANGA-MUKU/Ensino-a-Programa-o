...
if (isset($_GET['deletar'])){
    $id_desafio= intval($_GET['deletar']);
    $sql_query = $ligar->query("SELECT * FROM desafio WHERE id_desafio='$id_desafio'") or die ($ligar->error);
    $arquivos= $sql_query-> fetch_assoc();


 if (unlink($arquivos['Pasta'])){
    $deu_certo = $ligar->query("DELETE FROM desafio WHERE id_desafio='$id_desafio'") or die ($ligar->error);
    if($deu_certo)
    echo" <script> 
       
    alert('desafio apagado com sucesso');
...
function enviararquivo($error, $size, $name,$tmp_name){
    ...
    alert('Falha ao enviar ao desafio');
    window.location.href='Avaliacao.php';
    </script>");
    if($size > 2097152)
    die(" <script> 
...
if($deu_certo){
    $ligar->query("INSERT INTO desafio (Nome, Pasta) VALUES('$nomedoarquivo','$Pasta')") or die ($ligar->error);
return true;
}else
  return false;
}
...
$sql_query = $ligar->query("SELECT * FROM desafio") or die ($ligar->error);
