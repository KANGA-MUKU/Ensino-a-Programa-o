...
if (isset($_GET['deletar'])){
    $id_arquivo= intval($_GET['deletar']);
    $sql_query = $ligar->query("SELECT * FROM Php WHERE id_php='$id_php'") or die ($ligar->error);
    $arquivos= $sql_query-> fetch_assoc();


 if (unlink($arquivos['Pasta'])){
    $deu_certo = $ligar->query("DELETE FROM Php WHERE id_php='$id_php'") or die ($ligar->error);
    if($deu_certo)
    echo" <script> 
...
if($deu_certo){
    $ligar->query("INSERT INTO Php (nome, Pasta) VALUES('$nomedoarquivo','$Pasta')") or die ($ligar->error);
return true;
}else
  return false;
}
